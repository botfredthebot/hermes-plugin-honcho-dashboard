...
(function () {
  ...
  function PeersTab() {
    ...
    var _u7 = useState(null), selectedPeer = _u7[0], setSelectedPeer = _u7[1];
    var _u8 = useState(null), peerCardData = _u8[0], setPeerCardData = _u8[1];
    var _u9 = useState(false), cardLoading = _u9[0], setCardLoading = _u9[1];
    ...
    function loadPeerCard(peerId) {
      if (selectedPeer === peerId) {
        // Toggle off if already selected
        setSelectedPeer(null);
        setPeerCardData(null);
        return;
      }
      setSelectedPeer(peerId);
      setCardLoading(true);
      setPeerCardData(null);
      fetchJSON(API + "/peer/" + encodeURIComponent(peerId) + "/card")
        .then(function (d) { setPeerCardData(d); })
        .catch(function () { setPeerCardData({ error: "Failed to load peer card" }); })
        .finally(function () { setCardLoading(false); });
    }
      ...
      loading
        ...
        : peers.map(function (p) {
            ...
            return h("div", { key: p.id },
              h("div", { style: Object.assign({}, S.rowCard, isSelected ? { borderColor: "#2ea043", background: "rgba(46, 160, 67, 0.06)" } : null) },
                h("div", { style: S.rowInner },
                  ...
                  h("div", { style: S.rowRight },
                    h("button", {
                      title: "View peer card for '" + displayName + "'",
                      onClick: function (e) {
                        e.stopPropagation();
                        loadPeerCard(p.id);
                      },
                      style: isSelected ? Object.assign({}, S.btnSmall, { borderColor: "#2ea043", color: "#2ea043" }) : S.btnSmall,
              ...
              // Peer card panel
              isSelected
                ? h("div", { style: { marginBottom: 12, border: "1px solid #21262d", borderTop: "none", background: "rgba(22, 27, 34, 0.4)", padding: "14px 16px" } },
                    cardLoading
                      ? h("div", { style: { color: "#8b949e", fontSize: "0.82em" } }, "⏳ Loading peer card…")
                      : peerCardData && peerCardData.error
                      ? h("div", { style: { color: "#f85149", fontSize: "0.82em" } }, "⚠️ " + peerCardData.error)
                      : peerCardData && peerCardData.peer_card
                      ? h("div", null,
                          h("div", { style: { fontWeight: 600, fontSize: "0.88em", color: "#2ea043", marginBottom: 8 } }, "📋 Peer Card"),
                          h("pre", { style: { fontSize: "0.78em", color: "#c9d1d9", whiteSpace: "pre-wrap", wordBreak: "break-word", margin: 0, fontFamily: "monospace, monospace" } }, JSON.stringify(peerCardData.peer_card, null, 2))
                        )
                      : h("div", { style: { color: "#8b949e", fontSize: "0.82em" } }, "ℹ️ No peer card available for this peer yet. Peer cards are generated by Honcho during dream cycles.")
                  )
                : null,
              isDeleteTarget
